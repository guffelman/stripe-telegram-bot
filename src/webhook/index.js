...
const { bot, removeUserFromChannel } = require('../bot/events');
const { getCustomer, getCustomerByEmail, getCustomerMetadata, isCustomerActiveByEmail, updateCustomerTeleID} = require('./stripehelper.js');
...
async function handleStripeWebhook(req, res) {
  ...
  switch (event.type) {
      ...
      case 'customer.subscription.deleted':
        ...
        getCustomerMetadata(subscriptionDeleted.customer)
          .then((metadata) => {
            ...
            // Loop through channelIds array and remove user from each channel
    ...
    case 'charge.refunded':
      ...
      // teleid = getCustomerMetadata(chargeRefunded.customer).then((metadata) => metadata.teleid);
      // console.log(teleid)
      // // loop through ChannelIds array and remove user from each channel
      // channelIds.forEach((channelId) => {
      //   removeUserFromChannel(teleid, channelId);
      // });
